...
interface PrepareOfferForSigningDto {
  intake: ApsIntake;
  seller: {
    email: string;
    name: string;
...
export class OffersController {
   ...
   * POST /offers/:id/prepare-signature
   * Returns signing URL for embedded WebView
   */
  @Post(":id/prepare-signature")
  async prepareOfferForSigning(
    @Param("id") id: string,
    @Body() dto: PrepareOfferForSigningDto
  ) {
    const result = await this.offersService.prepareOfferForSigning(
      id,
      dto.intake,
      dto.seller
    );
  ...
  /**
   * @deprecated Use /prepare-signature instead
